/*
  ...
  这段代码改编自Lester Hedges的一个AABB树项目：
  https://github.com/lohedges/aabbcc

  改编后只支持2维，参数使用Vec2和ZY_Size，而非vector<float>
*/
#ifndef _AABB_H
#define _AABB_H
...
class Node;
    ...
    public:
        ...
        /// Get the surface area of the box. 求表面积
    ...
    public:
        ...
        //! Update the tree if a particle moves outside its fattened AABB. 如果粒子移动到增肥的AABB之外，更新树。 
        /*! \param particle
                The particle index (particleMap will be used to map the node). 粒子索引(particleMap将用于映射节点)。  

            \param position
                The position vector of the particle. 粒子的位置向量。

            \param radius
                The radius of the particle. 粒子的半径。

            \param alwaysReinsert
                Always reinsert the particle, even if it's within its old AABB (default:false) 总是重新插入粒子，即使它在旧的AABB中(默认值:false)  

            \return
                Whether the particle was reinserted. 粒子是否被重新插入。
         */
        ...
        //! Query the tree to find candidate interactions for a particle. 查询树以找到粒子的候选交互。  
        /*! \param particle
                The particle index.
        ...
        //! Query the tree to find candidate interactions for an AABB. 查询树以找到AABB的候选交互。  
        /*! \param particle
                The particle index.
        ...
        //! Compute the maximum balancance of the tree. 计算树的最大平衡。
        /*! \return
        ...
        //! Compute the surface area ratio of the tree. 计算树的表面积比。
        /*! \return
                The ratio of the sum of the node surface area to the surface
                area of the root node. 节点表面积之和与根节点表面积之和之比。
         */
        float computeSurfaceAreaRatio() const;

        /// Validate the tree. 验证树。
        void validate() const;

        /// Rebuild an optimal tree. 重建最优树。
        void rebuild();

    private:
        /// The index of the root node. 根节点下标
        unsigned int root;

        /// The dynamic tree. 动态树。
        std::vector<ABNode> nodes;

        /// The current number of nodes in the tree. 树中当前节点的数量。
        unsigned int nodeCount;

        /// The current node capacity. 当前节点容量。
        unsigned int nodeCapacity;

        /// The position of node at the top of the free list. 节点在空闲列表顶部的位置。
        unsigned int freeList;

        /// The dimensionality of the system. 系统的维数。
        //unsigned int dimension;

        /// Whether the system is periodic along at least one axis. 系统是否至少在一个轴上具有周期性。
        bool isPeriodic;

        /// The skin thickness of the fattened AABBs, as a fraction of the AABB base length. 脂肪化的AABB的皮肤厚度，作为AABB基底长度的一部分。
        float skinThickness;

        /// Whether the system is periodic along each axis.  系统是否沿每个轴呈周期性。
        bool periodicity[2];

        /// The size of the system in each dimension. 每个维度中系统的大小。
        ZY_Size boxSize;

        /// The position of the negative minimum image. 负的最小图像的位置。
        Vec2 negMinImage;

        /// The position of the positive minimum image. 正最小图像的位置。
        Vec2 posMinImage;

        /// A map between particle and node indices 粒子和节点索引之间的映射。
        std::unordered_map<Node*, unsigned int> particleMap; 
        /// Does touching count as overlapping in tree queries? 在树查询中触摸是否算作重叠?
        bool touchIsOverlap;
        ...
        //! Assert that the sub-tree has valid metrics. 断言子树具有有效的度量。
        /*! \param node
                The index of the root node.
         */
        ...
        //! Compute minimum image separation. 计算最小图像分离。
        /*! \param separation
                The separation vector. 分离向量

            \param shift
                The shift vector. 变化向量

            \return
                Whether a periodic shift has been applied. 是否应用了周期移位。
         */
