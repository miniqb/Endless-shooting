...
namespace neb
{
    ...
    int64 CJsonObject::getInt(const std::string& strKey)
    {
        cJSON* pJsonStruct = NULL;
        if (m_pJsonData != NULL)
        {
    ...
    int64 CJsonObject::getInt(unsigned int uiWhich)
    {
        cJSON* pJsonStruct = NULL;
        if (m_pJsonData != NULL)
        {
    ...
    bool CJsonObject::getNums(std::vector<int>& mp)
    {
        cJSON* pJsonStruct = NULL;
        if (m_pJsonData != NULL)
        {
            if (m_pJsonData->type == cJSON_Array)
            {
                pJsonStruct = cJSON_GetArrayItem(m_pJsonData, 0);
            }
        }
        else if (m_pExternJsonDataRef != NULL)
        {
            if (m_pExternJsonDataRef->type == cJSON_Array)
            {
                pJsonStruct = cJSON_GetArrayItem(m_pExternJsonDataRef, 0);
            }
        }
        if (pJsonStruct == NULL)
        {
        ...
        {
            do {
                mp.push_back(pJsonStruct->valueint);
